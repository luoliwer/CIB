...
- (void)_flushMessageQueue {
    ...
    for (WVJBMessage* message in messages) {
        ...
        NSArray* dataName = message[@"data"];
        NSString *functionName = nil;
        if ([dataName isKindOfClass:[NSString class]]) {
            functionName = dataName;
        }
        else if ([dataName isKindOfClass:[NSArray class]]) {
            functionName = [dataName objectAtIndex:0];
        }
        else {
            functionName = @"";
        }
        if (responseId) {
            ...
            NSArray *callbackIdArray = message[@"callbackIds"];
            NSString *theLastOne = [callbackIdArray objectAtIndex:(callbackIdArray.count - 1)];
            NSMutableArray *responseCallbackArray = [[NSMutableArray alloc] init];
            WVJBResponseCallback responseCallback = NULL;
            
            for (NSString *callbackId in callbackIdArray) {
                ...
                if (callbackId) {
                    ...
                    responseCallback = ^(id responseData) {
                        ...
                        if ([functionName isEqualToString:@"downloadFile"] && [callbackId isEqualToString:theLastOne]) {
                            WVJBMessage* msg = @{ @"responseId":callbackId, @"responseData":responseData,@"functionName":functionName,@"flag":@"no"};
                            [self _queueMessage:msg];
                        }else{
                            WVJBMessage* msg = @{ @"responseId":callbackId, @"responseData":responseData,@"functionName":functionName,@"flag":@"yes"};
                            [self _queueMessage:msg];
                        }
                ...
                [responseCallbackArray addObject:responseCallback];
            }
            ...
            handler(message[@"data"], responseCallbackArray);
        }
    }
}
